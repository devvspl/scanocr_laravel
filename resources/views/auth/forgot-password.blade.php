...
<div class="sm:mx-auto sm:w-full sm:max-w-md">
    ...
    <p class="mt-2 text-center text-sm text-stone-600 dark:text-stone-400">
        Enter your email to receive a 6-digit secure OTP.
    </p>
</div>
        ...
        <form class="space-y-6" action="{{ route('password.email') }}" method="POST" x-data="{ submitting: false }" @submit="submitting = true">
            ...
            <div class="pt-4">
                <button type="submit" x-bind:disabled="submitting"
                    ...
                    <span x-text="submitting ? 'Sending...' : 'Send OTP via Email'">Send OTP via Email</span>
                </button>
            </div>
            
            <div class="mt-4 text-center">
                <a href="{{ route('login') }}" class="text-sm font-medium text-red-900 hover:text-red-800 dark:text-red-400 transition-colors">
                    &larr; Back to login
                </a>
            </div>
        </form>
