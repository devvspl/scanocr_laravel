...
<div class="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
    ...
    <div class="bg-white py-8 px-4 shadow-xl sm:rounded-1xl sm:px-10 border border-stone-100 dark:bg-stone-900/80 dark:border-stone-800/50 backdrop-blur-md">
        <form class="space-y-6" action="{{ route('password.update') }}" method="POST" x-data="{ submitting: false }" @submit="submitting = true">
            @csrf
            ...
            <div class="pt-4">
                <button type="submit" x-bind:disabled="submitting"
                    ...
                    <span x-text="submitting ? 'Resetting...' : 'Set New Password'">Set New Password</span>
                </button>
            </div>
        </form>

        <!-- Resend OTP Form -->
        <form action="{{ route('password.otp.resend') }}" method="POST" class="mt-4" x-data="{ resending: false }" @submit="resending = true">
            @csrf
            <input type="hidden" name="email" value="{{ $email }}">
