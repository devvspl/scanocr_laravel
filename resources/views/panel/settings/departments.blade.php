...
@section('content')
<div x-data="departmentsPage()" x-init="init()">
    ...
    <div x-show="modalOpen"
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl overflow-hidden"
            ...
            <div class="px-5 py-4 border-b border-stone-100 flex items-center justify-between">
                <h3 class="text-sm font-semibold text-stone-800" x-text="editId ? 'Edit Department' : 'Add Department'"></h3>
            ...
            <div class="p-5">
                ...
                <div class="space-y-4">
                    <div>
                        <label class="block text-xs font-semibold text-stone-600 mb-1.5">Department Name <span class="text-red-600">*</span></label>
                        <input type="text" x-model="form.department_name" placeholder="e.g. Sales Department"
                        ...
                        <p x-show="errors.department_name" x-text="errors.department_name" class="mt-1 text-xs text-red-600"></p>
                    </div>
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-xs font-semibold text-stone-600 mb-1.5">Department Code <span class="text-red-600">*</span></label>
                            <input type="text" x-model="form.department_code" placeholder="e.g. SALES" maxlength="20"
                                   ...
                                   :class="errors.department_code ? 'border-red-400' : ''">
                            <p x-show="errors.department_code" x-text="errors.department_code" class="mt-1 text-xs text-red-600"></p>
                        </div>
                        <div>
                            ...
                            <select x-model="form.company_id"
                                    ...
                                    :class="errors.company_id ? 'border-red-400' : ''">
            ...
            <div class="px-5 py-4 border-t border-stone-100 flex items-center justify-end gap-2">
                ...
                <button @click="save()" :disabled="saving"
                    ...
                    <span x-text="editId ? 'Save Changes' : 'Add Department'"></span>
                </button>
            </div>
        </div>
    </div>
...
function departmentsPage() {
    return {
        ...
        resetForm() {
            this.form = { department_name: '', department_code: '', company_id: '', status: true };
        },
        ...
        async openModal(id = null) {
            ...
            if (id) {
                this.editId = id;
                const res  = await fetch(`/settings/departments/${id}`, {
        ...
        async save() {
            ...
            const url    = this.editId ? `/settings/departments/${this.editId}` : '/settings/departments';
        ...
        initTable() {
            ...
            const table = $('#dt-table').DataTable({
                ...
                ajax: {
                    url: '/settings/departments/data',
                    data(d) { d.status = $('#dt-filter-status').val(); },
                },
                columns: [
                    { data: 'department_id', className: 'td-center text-stone-400 text-xs' },
                    { data: 'department_name', render: v => `<span class="font-medium text-stone-800">${v}</span>` },
                    { data: 'department_code', render: v => `<code class="text-xs bg-stone-100 px-1.5 py-0.5 rounded font-mono text-stone-600">${v}</code>` },
                    ...
                    {
                        data: 'department_id', orderable: false, className: 'td-center',
                        render(id) {
                ...
                language: {
                    emptyTable: '<div class="py-12 text-center text-sm text-stone-400">No departments found</div>',
                },
            });
