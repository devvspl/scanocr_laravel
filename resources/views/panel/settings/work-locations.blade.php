...
@section('content')
<div x-data="workLocationsPage()" x-init="init()">
    ...
    <div x-show="modalOpen"
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl overflow-hidden"
            ...
            <div class="px-5 py-4 border-b border-stone-100 flex items-center justify-between">
                <h3 class="text-sm font-semibold text-stone-800" x-text="editId ? 'Edit Work Location' : 'Add Work Location'"></h3>
            ...
            <div class="p-5">
                ...
                <div class="space-y-4">
                    <div>
                        <label class="block text-xs font-semibold text-stone-600 mb-1.5">Location Name <span class="text-red-600">*</span></label>
                        <input type="text" x-model="form.location_name" placeholder="e.g. Head Office"
                               ...
                               :class="errors.location_name ? 'border-red-400' : ''">
                        <p x-show="errors.location_name" x-text="errors.location_name" class="mt-1 text-xs text-red-600"></p>
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-stone-600 mb-1.5">Location Code <span class="text-red-600">*</span></label>
                        <input type="text" x-model="form.location_code" placeholder="e.g. HO001" maxlength="50"
                               ...
                               :class="errors.location_code ? 'border-red-400' : ''">
                        <p x-show="errors.location_code" x-text="errors.location_code" class="mt-1 text-xs text-red-600"></p>
                    </div>
            ...
            <div class="px-5 py-4 border-t border-stone-100 flex items-center justify-end gap-2">
                ...
                <button @click="save()" :disabled="saving"
                    ...
                    <span x-text="editId ? 'Save Changes' : 'Add Location'"></span>
                </button>
            </div>
        </div>
    </div>
...
function workLocationsPage() {
    return {
        ...
        resetForm() {
            this.form = { location_name: '', location_code: '', status: true };
        },
        ...
        async openModal(id = null) {
            ...
            if (id) {
                this.editId = id;
                const res  = await fetch(`/settings/work-locations/${id}`, {
        ...
        async save() {
            ...
            const url    = this.editId ? `/settings/work-locations/${this.editId}` : '/settings/work-locations';
        ...
        initTable() {
            ...
            const table = $('#dt-table').DataTable({
                ...
                ajax: {
                    url: '/settings/work-locations/data',
                    data(d) { d.status = $('#dt-filter-status').val(); },
                },
                columns: [
                    { data: 'location_id', className: 'td-center text-stone-400 text-xs' },
                    { data: 'location_name', render: v => `<span class="font-medium text-stone-800">${v}</span>` },
                    { data: 'location_code', render: v => `<code class="text-xs bg-stone-100 px-1.5 py-0.5 rounded font-mono text-stone-600">${v}</code>` },
                    {
                    ...
                    {
                        data: 'location_id', orderable: false, className: 'td-center',
                        render(id) {
                ...
                language: {
                    emptyTable: '<div class="py-12 text-center text-sm text-stone-400">No work locations found</div>',
                },
            });
