...
@section('content')
<div x-data="ledgersPage()" x-init="init()">
    ...
    <div x-show="modalOpen"
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl overflow-hidden"
            ...
            <div class="px-5 py-4 border-b border-stone-100 flex items-center justify-between">
                <h3 class="text-sm font-semibold text-stone-800" x-text="editId ? 'Edit Ledger' : 'Add Ledger'"></h3>
            ...
            <div class="p-5">
                ...
                <div class="space-y-4">
                    <div>
                        <label class="block text-xs font-semibold text-stone-600 mb-1.5">Ledger Name <span class="text-red-600">*</span></label>
                        <input type="text" x-model="form.ledger_name" placeholder="e.g. Cash Account"
                               ...
                               :class="errors.ledger_name ? 'border-red-400' : ''">
                        <p x-show="errors.ledger_name" x-text="errors.ledger_name" class="mt-1 text-xs text-red-600"></p>
                    </div>
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-xs font-semibold text-stone-600 mb-1.5">Ledger Code <span class="text-red-600">*</span></label>
                            <input type="text" x-model="form.ledger_code" placeholder="e.g. L001" maxlength="20"
                                   ...
                                   :class="errors.ledger_code ? 'border-red-400' : ''">
                            <p x-show="errors.ledger_code" x-text="errors.ledger_code" class="mt-1 text-xs text-red-600"></p>
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-stone-600 mb-1.5">Ledger Head <span class="text-red-600">*</span></label>
                            <input type="text" x-model="form.ledger_head" placeholder="e.g. Assets" maxlength="20"
                                   class="w-full border border-stone-300 rounded-xl px-3.5 py-2.5 text-sm text-stone-800 placeholder-stone-400 focus:outline-none focus:border-red-700 focus:ring-2 focus:ring-red-700/10 transition-colors"
                                   :class="errors.ledger_head ? 'border-red-400' : ''">
                            <p x-show="errors.ledger_head" x-text="errors.ledger_head" class="mt-1 text-xs text-red-600"></p>
                        </div>
                    </div>
            ...
            <div class="px-5 py-4 border-t border-stone-100 flex items-center justify-end gap-2">
                ...
                <button @click="save()" :disabled="saving"
                    ...
                    <span x-text="editId ? 'Save Changes' : 'Add Ledger'"></span>
                </button>
            </div>
        </div>
    </div>
...
function ledgersPage() {
    return {
        ...
        resetForm() {
            this.form = { ledger_name: '', ledger_code: '', ledger_head: '', status: true };
        },
        ...
        async openModal(id = null) {
            ...
            if (id) {
                this.editId = id;
                const res  = await fetch(`/settings/ledgers/${id}`, {
        ...
        async save() {
            ...
            const url    = this.editId ? `/settings/ledgers/${this.editId}` : '/settings/ledgers';
        ...
        initTable() {
            ...
            const table = $('#dt-table').DataTable({
                ...
                ajax: {
                    url: '/settings/ledgers/data',
                    data(d) { d.status = $('#dt-filter-status').val(); },
                },
                columns: [
                    { data: 'ledger_id', className: 'td-center text-stone-400 text-xs' },
                    { data: 'ledger_name', render: v => `<span class="font-medium text-stone-800">${v}</span>` },
                    { data: 'ledger_code', render: v => `<code class="text-xs bg-stone-100 px-1.5 py-0.5 rounded font-mono text-stone-600">${v}</code>` },
                    { data: 'ledger_head', render: v => `<span class="text-sm text-stone-600">${v}</span>` },
                    {
                    ...
                    {
                        data: 'ledger_id', orderable: false, className: 'td-center',
                        render(id) {
                ...
                language: {
                    emptyTable: '<div class="py-12 text-center text-sm text-stone-400">No ledgers found</div>',
                },
            });
