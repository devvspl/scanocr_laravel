...
<div x-data="fyPage()" x-init="init()">
    ...
    <div class="bg-white border border-stone-200 rounded-1xl px-4 py-3 mb-4 flex items-center gap-3">
        ...
        <div class="flex flex-wrap gap-2">
            ...
            <a href="{{ route('settings.financial-year', ['company_id' => $co->id]) }}"
               class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-colors
                      {{ $co->id == $selectedId ? 'bg-red-800 text-white' : 'bg-stone-100 text-stone-600 hover:bg-stone-200' }}">
                @if($co->is_default)
        ...
        async save() {
            ...
            try {
                const res = await fetch(url, {
                    ...
                    body: JSON.stringify({ ...this.form, company_id: this.companyId }),
                });
