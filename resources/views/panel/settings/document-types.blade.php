...
@section('content')
<div x-data="docTypesPage()" x-init="init()">
    ...
    <div class="bg-white border border-stone-200 rounded-1xl overflow-hidden">
        ...
        <div class="px-4 py-2.5 border-b border-stone-100 flex items-center justify-between gap-2 min-h-[48px]">
            <div class="flex items-center gap-2">
                ...
                <select id="dt-filter-approval" class="h-7 px-2.5 text-[11px] border border-stone-200 rounded-md text-stone-600 bg-stone-50 focus:outline-none focus:border-red-700 focus:ring-1 focus:ring-red-700/10">
                    <option value="">All Approval</option>
                    <option value="enabled">Approval Enabled</option>
                    <option value="disabled">Approval Disabled</option>
                </select>
            </div>
        ...
        <div class="overflow-x-auto">
            <table id="dt-table" class="w-full">
                <thead><tr>
                    ...
                    <th>Label</th>
                    <th>Key</th>
                    <th class="td-center">Default Prefix</th>
                    <th>Module</th>
                    <th>Created By</th>
                    <th class="dt-center">Approval</th>
    ...
    <div x-show="modalOpen"
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl overflow-hidden"
            ...
            <div class="px-5 py-4 border-b border-stone-100 flex items-center justify-between">
                <h3 class="text-sm font-semibold text-stone-800" x-text="editId ? 'Edit Document Type' : 'Add Document Type'"></h3>
                ...
                <div class="space-y-4">
                    <div class="grid grid-cols-2 gap-4">
                        <div class="col-span-2">
                            ...
                            <input type="text" x-model="form.label" placeholder="e.g. Sales Invoice"
                        ...
                        <div x-show="!editId">
                            <label class="block text-xs font-semibold text-stone-600 mb-1.5">Key <span class="text-red-600">*</span></label>
                            <input type="text" x-model="form.key" placeholder="e.g. invoice"
                                   ...
                                   :class="errors.key ? 'border-red-400' : ''"
                                   @input="form.key = form.key.toLowerCase().replace(/[^a-z0-9_]/g, '_')">
                            <p x-show="errors.key" x-text="errors.key" class="mt-1 text-xs text-red-600"></p>
                            <p class="text-[10px] text-stone-400 mt-1">Lowercase, numbers, underscores only</p>
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-stone-600 mb-1.5">Default Prefix <span class="text-red-600">*</span></label>
                            <input type="text" x-model="form.default_prefix" placeholder="e.g. INV" maxlength="20"
                                   class="w-full border border-stone-300 rounded-xl px-3.5 py-2.5 text-sm text-stone-800 placeholder-stone-400 font-mono focus:outline-none focus:border-red-700 focus:ring-2 focus:ring-red-700/10 transition-colors"
                                   :class="errors.default_prefix ? 'border-red-400' : ''">
                            <p x-show="errors.default_prefix" x-text="errors.default_prefix" class="mt-1 text-xs text-red-600"></p>
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-stone-600 mb-1.5">Module</label>
                            <select x-model="form.module"
                        ...
                        <div>
                            <label class="block text-xs font-semibold text-stone-600 mb-1.5">Sort Order</label>
                    ...
                    <div style="display:flex;align-items:center;gap:12px;margin-top:6px;">
                        <span @click="form.digital_approval = !form.digital_approval"
                            :style="'display:block;width:36px;min-width:36px;height:20px;border-radius:10px;position:relative;cursor:pointer;transition:background .2s;background:' + (form.digital_approval ? '#16a34a' : '#d6d3d1')">
                            <span :style="'position:absolute;top:2px;left:' + (form.digital_approval ? '18px' : '2px') + ';width:16px;height:16px;background:#fff;border-radius:50%;box-shadow:0 1px 3px rgba(0,0,0,.15);transition:left .2s;display:block;'"></span>
                        </span>
                        <span style="font-size:14px;font-weight:500;" :style="'color:' + (form.digital_approval ? '#16a34a' : '#57534e')" x-text="form.digital_approval ? 'Digital Approval — Enabled' : 'Digital Approval — Disabled'"></span>
                    </div>
                </div>
            </div>
            ...
            <div class="px-5 py-4 border-t border-stone-100 flex items-center justify-end gap-2">
                ...
                <button @click="save()" :disabled="saving"
                    ...
                    <span x-text="editId ? 'Save Changes' : 'Add Document Type'"></span>
                </button>
            </div>
        </div>
    </div>
...
function docTypesPage() {
    return {
        ...
        resetForm() {
            this.form = { key: '', label: '', default_prefix: '', module: '', sort_order: 0, is_active: true, digital_approval: false };
        },
        ...
        async openModal(id = null) {
            ...
            if (id) {
                this.editId = id;
                const res  = await fetch(`/settings/document-types/${id}`, {
                ...
                this.form  = {
                    ...
                    digital_approval: data.digital_approval ?? false,
                };
            } else {
        ...
        async save() {
            ...
            const url    = this.editId ? `/settings/document-types/${this.editId}` : '/settings/document-types';
            const method = this.editId ? 'PUT' : 'POST';
            try {
                const res = await fetch(url, {
                    method,
                    headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content, 'Accept': 'application/json' },
                    body: JSON.stringify({ ...this.form, is_active: this.form.is_active ? 1 : 0, digital_approval: this.form.digital_approval ? 1 : 0 }),
                });
        ...
        initTable() {
            ...
            const table = $('#dt-table').DataTable({
                ...
                ajax: {
                    url: '/settings/document-types/data',
                    data(d) { d.status = $('#dt-filter-status').val(); d.approval = $('#dt-filter-approval').val(); },
                },
                columns: [
                    { data: 'sort_order', className: 'td-center text-stone-400 text-xs' },
                    {
                        data: 'label',
                        render(v, t, row) {
                            const sys = row.is_system ? ' <span class="inline-flex items-center px-1.5 py-0.5 rounded text-[9px] font-bold bg-stone-100 text-stone-500 uppercase tracking-wide">System</span>' : '';
                            return `<span class="font-medium text-stone-800">${v}</span>${sys}`;
                        }
                    },
                    { data: 'key', render: v => `<code class="text-xs bg-stone-100 px-1.5 py-0.5 rounded font-mono text-stone-600">${v}</code>` },
                    { data: 'default_prefix', className: 'td-center', render: v => `<span class="font-mono font-bold text-red-700">${v}</span>` },
                    {
                        data: 'module',
                        render(v) {
                            if (!v) return '<span class="text-stone-400">—</span>';
                            const map = { sales: 'bg-blue-50 text-blue-700', purchase: 'bg-orange-50 text-orange-700', journal: 'bg-purple-50 text-purple-700', inventory: 'bg-green-50 text-green-700', other: 'bg-stone-100 text-stone-600' };
                            return `<span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold capitalize ${map[v] ?? 'bg-stone-100 text-stone-600'}">${v}</span>`;
                        }
                    },
                    {
                    ...
                    {
                        data: 'digital_approval', className: 'td-center',
                        render: v => v
                            ? '<span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold" style="background:#dcfce7;color:#166534;">Enabled</span>'
                            : '<span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold bg-stone-100 text-stone-500">Disabled</span>',
                    },
                    {
                    ...
                    },
                    {
                        ...
                        render(id, t, row) {
                            const del = row.is_system
                                ? `<button class="act-btn act-delete opacity-30 cursor-not-allowed" disabled title="System types cannot be deleted"><svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg></button>`
                                : `<button class="act-btn act-delete btn-delete" data-id="${id}"><svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg></button>`;
                            return `<div class="act-group justify-center"><button class="act-btn act-edit btn-edit" data-id="${id}"><svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg></button>${del}</div>`;
                        }
                    },
                ],
                ...
                language: {
                    emptyTable: '<div class="py-12 text-center text-sm text-stone-400">No document types found</div>',
                },
            });

            window._dtTable = table;
            $('#dt-filter-status, #dt-filter-approval').on('change', () => table.ajax.reload(null, false));
