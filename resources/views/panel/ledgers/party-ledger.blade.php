...
@section('title', ($party->type === 'customer' ? 'Customer' : 'Vendor') . ' Ledger - ' . ($party->display_name ?? $party->name))
@section('page-title', ($party->type === 'customer' ? 'Customer' : 'Vendor') . ' Ledger')
...
@section('breadcrumb')
    ...
    <a href="{{ route('ledgers.parties', ['type' => $party->type]) }}" class="hover:text-stone-600 transition-colors">
        {{ $party->type === 'customer' ? 'Customers' : 'Vendors' }}
    </a>
    <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
    </svg>
    <span class="text-stone-600">{{ $party->display_name ?? $party->name }}</span>
@endsection
...
<div class="bg-white border border-stone-200 rounded-xl overflow-hidden">
    ...
    <div class="border-b border-stone-200 bg-stone-50 px-6 py-4">
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-xl font-bold text-stone-800">{{ $party->display_name ?? $party->name }}</h2>
                <div class="flex items-center gap-4 mt-1">
                    <p class="text-sm text-stone-500">Code: {{ $party->code }}</p>
                    @if($party->gstin)
                        <p class="text-sm text-stone-500">GSTIN: {{ $party->gstin }}</p>
                    @endif
                    @if($party->phone || $party->mobile)
                        <p class="text-sm text-stone-500">Phone: {{ $party->phone ?? $party->mobile }}</p>
                    @endif
                </div>
            </div>
            ...
            <tbody>
                ...
                <tr class="border-b border-stone-100 hover:bg-stone-50 transition-colors">
                    ...
                    <td class="px-4 py-3">
                        ...
                        @if($entry->account)
                            <div class="text-xs text-stone-500">{{ $entry->account->name }}</div>
                        @endif
