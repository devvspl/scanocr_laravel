...
.ws-dot.done{background:#16a34a;color:#fff}.ws-dot.active{background:#7f1d1d;color:#fff;box-shadow:0 0 0 3px rgba(127,29,29,.18)}.ws-dot.pending{background:#e7e5e4;color:#a8a29e}
.ws-line{flex:1;height:2px;background:#e7e5e4;margin:0 .6rem}.ws-line.done{background:#16a34a}
.ws-label{font-size:.68rem;font-weight:500;white-space:nowrap}.ws-label.active{color:#7f1d1d}.ws-label.pending{color:#a8a29e}.ws-label.done{color:#16a34a}
...
.drop-zone:hover,.drop-zone.dragover{border-color:#b91c1c;background:rgba(185,28,28,.04)}.drop-zone.has-file{border-color:#16a34a;background:rgba(22,163,74,.04)}
.select2-container--default .select2-selection--single{height:2.25rem;border:1px solid #d6d3d1;border-radius:.5rem;background:#fafaf9;display:flex;align-items:center}
.select2-container--default .select2-selection--single .select2-selection__rendered{font-size:.75rem;color:#292524;padding-left:.75rem;line-height:2.25rem}
.select2-container--default .select2-selection--single .select2-selection__arrow{height:2.25rem;right:.5rem}
.select2-container--default .select2-selection--single .select2-selection__placeholder{color:#a8a29e}
.select2-container--default .select2-results__option{font-size:.75rem;padding:.4rem .75rem}
.select2-container--default .select2-results__option--highlighted{background:#7f1d1d;color:#fff}
...
.select2-container--open .select2-selection--single{border-color:#7f1d1d;box-shadow:0 0 0 3px rgba(127,29,29,.08)}
...
.badge-yes,.badge-approved{background:#dcfce7;color:#15803d}.badge-no{background:#f5f5f4;color:#78716c}.badge-rejected{background:#fee2e2;color:#b91c1c}.badge-pending{background:#fef9c3;color:#a16207}
#scansTable{border-collapse:collapse;width:100% !important;table-layout:fixed}
#scansTable thead th{background:#fafaf9;color:#78716c;font-size:.65rem;font-weight:700;text-transform:uppercase;letter-spacing:.04em;padding:.55rem .75rem;border-bottom:2px solid #e7e5e4;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;position:sticky;top:0;z-index:1}
#scansTable thead th.sorting:after,#scansTable thead th.sorting_asc:after,#scansTable thead th.sorting_desc:after{font-size:.55rem;opacity:.6;margin-left:.2rem}
#scansTable tbody td{padding:.55rem .75rem;border-bottom:1px solid #f0eeec;color:#292524;vertical-align:middle;font-size:.73rem;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
#scansTable tbody tr:hover td{background:#fafaf9}#scansTable tbody tr:last-child td{border-bottom:none}
...
.dataTables_processing{font-size:.72rem;color:#7f1d1d;padding:.5rem 0;text-align:center}
#scansTable_wrapper>.dataTables_length,#scansTable_wrapper>.dataTables_filter,#scansTable_wrapper>.dataTables_info,#scansTable_wrapper>.dataTables_paginate{display:none !important}
...
.dt-btn.blue:hover{background:#eff6ff;color:#2563eb}.dt-btn.green:hover{background:#f0fdf4;color:#16a34a}.dt-btn.red:hover{background:#fef2f2;color:#dc2626}
...
.log-row{display:flex;align-items:center;justify-content:space-between;gap:.75rem;padding:.6rem 0;border-bottom:1px solid #f5f5f4}.log-row:last-child{border-bottom:none}
.sf-row{display:flex;align-items:center;justify-content:space-between;gap:.75rem;padding:.6rem 1rem;border-bottom:1px solid #f5f5f4}.sf-row:last-child{border-bottom:none}
.step2-grid{display:grid;grid-template-columns:1fr;gap:1.25rem;align-items:start}@media(min-width:1024px){.step2-grid{grid-template-columns:minmax(0,1.8fr) minmax(0,1fr)}}
.file-viewer{width:100%;background:#1c1917;border-radius:.75rem;overflow:hidden;position:relative;min-height:480px;display:flex;flex-direction:column}
...
.file-viewer-body{flex:1;position:relative;min-height:440px}
.file-viewer-body iframe,.file-viewer-body img{position:absolute;inset:0;width:100%;height:100%;border:none;object-fit:contain;background:#1c1917}
...
.col-scroll::-webkit-scrollbar{width:4px}.col-scroll::-webkit-scrollbar-track{background:transparent}.col-scroll::-webkit-scrollbar-thumb{background:#d6d3d1;border-radius:2px}
...
.modal-viewer-section{flex:1;min-height:0;display:flex;flex-direction:column;background:#1c1917;border-top:1px solid #e7e5e4}
...
.modal-files-section{padding:1rem 1.25rem;border-top:1px solid #e7e5e4;background:#fafaf9;max-height:180px;overflow-y:auto;flex-shrink:0}
.modal-file-group{margin-bottom:1rem}.modal-file-group:last-child{margin-bottom:0}
.modal-file-group-title{font-size:.7rem;font-weight:700;color:#78716c;text-transform:uppercase;letter-spacing:.04em;margin-bottom:.5rem}
.modal-file-item{display:flex;align-items:center;gap:.5rem;padding:.4rem .75rem;border-radius:.5rem;cursor:pointer;transition:all .15s;margin-bottom:.2rem}
.modal-file-item:hover{background:#fff;box-shadow:0 1px 3px rgba(0,0,0,.08)}.modal-file-item.active{background:#fff;box-shadow:0 2px 6px rgba(0,0,0,.12);border-left:3px solid #7f1d1d}
.modal-file-icon{width:1.75rem;height:1.75rem;display:flex;align-items:center;justify-content:center;background:#f5f5f4;border-radius:.375rem;color:#78716c;font-size:9px;font-weight:700;text-transform:uppercase;flex-shrink:0;border:1px solid #e7e5e4}
.modal-file-name{flex:1;min-width:0;font-size:.72rem;color:#292524;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
</style>
@endpush
@section('content')
<div id="tempScanApp">
    <div class="wizard-panel active" id="step1">
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
            <div class="bg-white border border-stone-200 rounded-xl p-5 flex flex-col gap-4">
                <div><h2 class="text-sm font-semibold text-stone-800">Scan File</h2><p class="text-xs text-stone-400 mt-0.5">Upload a new temp scan document</p></div>
                <div class="wizard-steps bg-stone-50 rounded-lg border border-stone-100 px-3 py-2.5">
                    <div class="ws-item"><div class="ws-dot active" id="ws-dot-1">1</div><span class="ws-label active" id="ws-lbl-1">Upload Scan</span></div>
                    <div class="ws-line" id="ws-line-1"></div>
                    <div class="ws-item"><div class="ws-dot pending" id="ws-dot-2">2</div><span class="ws-label pending" id="ws-lbl-2">Supporting Files</span></div>
                </div>
                ...
                <form id="uploadForm" class="flex flex-col gap-3.5" novalidate>
                    @csrf
                    <div><label class="block text-xs font-medium text-stone-600 mb-1">Location <span class="text-red-500">*</span></label><select id="sel-location" name="location" style="width:100%"><option value="">Select Location</option></select></div>
                    <div><label class="block text-xs font-medium text-stone-600 mb-1">Bill Approver <span class="text-red-500">*</span></label><select id="sel-approver" name="bill_approver" style="width:100%" disabled><option value="">Select Approver</option></select></div>
                    <div><label class="block text-xs font-medium text-stone-600 mb-1">File <span class="text-red-500">*</span></label>
                        <div class="drop-zone" id="dropZone">
                            <input type="file" id="mainFile" name="main_file" accept=".jpg,.jpeg,.png,.pdf" class="hidden">
                            <svg class="w-7 h-7 text-stone-300 mx-auto mb-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12"/></svg>
                            <p class="text-xs text-stone-500" id="dropLabel">Drag &amp; drop or click</p>
                            <p class="text-[10px] text-stone-400 mt-0.5">JPG, PNG, PDF — max 40 MB</p>
                        </div>
                    </div>
                    <div id="uploadProgressWrap" class="hidden"><div class="upload-progress"><div class="upload-progress-bar" id="uploadProgressBar"></div></div><p class="text-[10px] text-stone-400 mt-1" id="uploadProgressText">Uploading…</p></div>
                    <button type="submit" id="uploadBtn" class="w-full h-9 bg-stone-800 hover:bg-stone-900 text-white text-xs font-semibold rounded-lg transition-colors flex items-center justify-center gap-2 disabled:opacity-50 mt-1">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"/></svg>Save &amp; Continue
                    </button>
                </form>
            </div>
                ...
                <div class="px-5 py-3.5 border-b border-stone-100 flex items-center justify-between gap-3 flex-shrink-0">
                    ...
                    <div class="flex items-center gap-2">
                        <button id="btnOpenLog" class="h-8 px-3 flex items-center gap-1.5 text-xs font-medium border border-stone-200 rounded-lg text-stone-600 hover:bg-stone-50 transition-colors">
                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"/></svg>Export Log
                        </button>
                        <div class="export-menu" id="exportMenu">
                            ...
                            <div class="export-drop">
                                <a href="{{ route('workflow.temp-scan.export.excel') }}" id="exportExcel"><svg class="w-3.5 h-3.5 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414A1 1 0 0119 9v10a2 2 0 01-2 2z"/></svg>Export Excel</a>
                                <a href="{{ route('workflow.temp-scan.export.pdf') }}" id="exportPdf"><svg class="w-3.5 h-3.5 text-red-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z"/></svg>Export PDF</a>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="dt-ctrl-bar border-b border-stone-100 flex-shrink-0">
                    <div class="flex items-center gap-2 text-xs text-stone-500"><span>Show</span><select class="dt-length-sel" id="dtLength"><option value="10">10</option><option value="25">25</option><option value="50">50</option><option value="100">100</option></select><span>entries</span></div>
                    <div class="dt-search-wrap"><input type="text" class="dt-search-input" id="dtSearch" placeholder="Search…"></div>
                </div>
                <div class="overflow-x-auto flex-1"><table id="scansTable" style="width:100%"><thead><tr><th>#</th><th>Location</th><th>File</th><th>Scan Date</th><th>Final Submit</th><th>Bill Approved</th><th>Approver</th><th>Remark</th><th>Action</th></tr></thead></table></div>
                <div class="dt-ctrl-bar border-t border-stone-100 flex-shrink-0" id="dtBottomBar"><div id="dtInfo" class="dataTables_info"></div><div id="dtPaginate"></div></div>
            </div>
        </div>
    </div>
    <div class="wizard-panel" id="step2">
        <div class="bg-white border border-stone-200 rounded-xl px-4 py-3 flex items-center justify-between gap-4 mb-4 flex-wrap">
            <div class="flex items-center gap-3 min-w-0">
                <div class="flex items-center gap-1.5 shrink-0"><div class="ws-dot done text-[.6rem]">✓</div><span class="ws-label done text-[.68rem]">Upload Scan</span><div class="ws-line done w-8"></div><div class="ws-dot active text-[.65rem]">2</div><span class="ws-label active text-[.68rem]">Supporting Files</span></div>
                <div class="w-px h-5 bg-stone-200 shrink-0 hidden sm:block"></div>
                <div class="flex items-center gap-2 min-w-0">
                    <div class="w-7 h-7 flex items-center justify-center bg-red-50 rounded-lg text-red-700 shrink-0"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586 a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg></div>
                    <div class="min-w-0"><p class="text-xs font-semibold text-stone-800 leading-tight" id="bannerScanId">—</p><p class="text-[10px] text-stone-400 leading-tight truncate" id="bannerScanMeta">—</p></div>
                </div>
            </div>
            <div class="flex items-center gap-2 shrink-0">
                <button id="btnFinalSubmit" class="h-8 px-3 bg-green-600 hover:bg-green-700 text-white text-xs font-semibold rounded-lg transition-colors flex items-center gap-1.5"><svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>Final Submit</button>
                <button id="btnBackToStep1" class="h-8 px-3 border border-stone-200 text-stone-600 hover:bg-stone-50 text-xs font-medium rounded-lg transition-colors flex items-center gap-1"><svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>Back</button>
            </div>
        </div>
        <div class="step2-grid">
            <div class="file-viewer">
                <div class="file-viewer-toolbar"><span class="text-[10px] font-semibold text-stone-300 uppercase tracking-wide">Scan Preview</span><a id="viewerOpenLink" href="#" target="_blank" class="text-[10px] text-stone-400 hover:text-white transition-colors flex items-center gap-1"><svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"/></svg>Open in new tab</a></div>
                <div class="file-viewer-body" id="fileViewerBody"><div class="viewer-placeholder" id="viewerPlaceholder"><svg class="w-12 h-12" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586 a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg><p class="text-xs">No file loaded</p></div></div>
            </div>
            <div class="col-scroll">
                <div class="bg-white border border-stone-200 rounded-xl p-4 flex flex-col gap-3.5 mb-4">
                    <div class="flex items-center gap-2"><div class="w-6 h-6 flex items-center justify-center bg-stone-100 rounded-md text-stone-500 shrink-0"><svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg></div><h3 class="text-sm font-semibold text-stone-800">Add Supporting File</h3></div>
                    ...
                    <form id="supportForm" class="flex flex-col gap-3" novalidate>
                        @csrf
                        <div><label class="block text-xs font-medium text-stone-600 mb-1">Document Type</label><select id="sel-doctype" name="doc_type_id" style="width:100%"><option value="">— Select —</option></select></div>
                        <div><label class="block text-xs font-medium text-stone-600 mb-1">File <span class="text-red-500">*</span></label>
                            <div class="drop-zone" id="supportDropZone">
                                <input type="file" id="supportFile" name="support_file" accept=".jpg,.jpeg,.png,.pdf" class="hidden">
                                <svg class="w-6 h-6 text-stone-300 mx-auto mb-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12"/></svg>
                                <p class="text-xs text-stone-500" id="supportDropLabel">Drag &amp; drop or click</p><p class="text-[10px] text-stone-400 mt-0.5">JPG, PNG, PDF — max 40 MB</p>
                            </div>
                        </div>
                        <div id="supportProgressWrap" class="hidden"><div class="upload-progress"><div class="upload-progress-bar" id="supportProgressBar"></div></div><p class="text-[10px] text-stone-400 mt-1" id="supportProgressText">Uploading…</p></div>
                        <button type="submit" id="supportUploadBtn" class="w-full h-9 bg-stone-800 hover:bg-stone-900 text-white text-xs font-semibold rounded-lg transition-colors flex items-center justify-center gap-2 disabled:opacity-50"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"/></svg>Upload File</button>
                    </form>
                    <div class="bg-stone-50 rounded-lg border border-stone-100 px-3 py-2.5"><p class="text-[10px] text-stone-500 leading-relaxed">Attach supporting documents — invoices, receipts, approval forms, etc.</p></div>
                </div>
                <div class="bg-white border border-stone-200 rounded-xl overflow-hidden">
                    <div class="px-4 py-3 border-b border-stone-100 flex items-center justify-between">
                        <div class="flex items-center gap-2"><div class="w-6 h-6 flex items-center justify-center bg-stone-100 rounded-md text-stone-500 shrink-0"><svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.586 a4 4 0 00-5.656-5.656l-6.415 6.585a6 6 0 108.486 8.486L20.5 13"/></svg></div><h3 class="text-sm font-semibold text-stone-800">Supporting Files</h3></div>
                        <span id="supportCountBadge" class="hidden text-[10px] font-semibold bg-stone-100 text-stone-600 px-2 py-0.5 rounded-full"></span>
                    </div>
                    <div id="supportList"><div class="flex flex-col items-center justify-center py-10 gap-2 text-stone-400"><svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.2" d="M9 13h6m-3-3v6m-9 1V7a2 2 0 012-2h6l2 2h6a2 2 0 012 2v8 a2 2 0 01-2 2H5a2 2 0 01-2-2z"/></svg><p class="text-xs">No supporting files yet</p></div></div>
                </div>
            </div>
        </div>
    </div>
...
<div class="modal-container" id="viewScanModal">
    <div class="modal-dialog">
        <div class="modal-header">
            <div><h3 class="text-sm font-semibold text-stone-800" id="modalScanTitle">—</h3><p class="text-xs text-stone-400 mt-0.5" id="modalScanMeta">—</p></div>
            <button id="btnCloseModal" class="w-7 h-7 flex items-center justify-center rounded-lg text-stone-400 hover:bg-stone-100 transition-colors"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg></button>
        </div>
        <div class="modal-body">
            <div class="modal-viewer-section">
                <div class="modal-viewer-toolbar"><span id="modalViewingFileName" class="text-[10px] font-semibold text-stone-300">Main Scan</span></div>
                <div class="modal-viewer-body" id="modalViewerBody"></div>
            </div>
            <div class="modal-files-section" id="modalFilesSection">
                <div class="modal-file-group"><div class="modal-file-group-title">Main Scan</div><div id="modalMainFile"></div></div>
                <div class="modal-file-group" id="modalSupportingGroup" style="display:none"><div class="modal-file-group-title">Supporting Files</div><div id="modalSupportingFiles"></div></div>
            </div>
        </div>
    </div>
</div>
...
<div id="logCanvas">
    <div class="lc-header"><div><h3 class="text-sm font-semibold text-stone-800">Export Log</h3><p class="text-xs text-stone-400 mt-0.5">Your recent temp scan exports</p></div>
        <button id="btnCloseLog" class="w-7 h-7 flex items-center justify-center rounded-lg text-stone-400 hover:bg-stone-100 transition-colors"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg></button>
    </div>
    <div class="lc-body" id="logBody"><p class="text-center text-xs text-stone-400 py-8">Loading…</p></div>
</div>
@endsection
@push('scripts')
<script>
...
const R={locations:'{{route("workflow.temp-scan.locations")}}',approvers:'{{route("workflow.temp-scan.bill-approvers")}}',docTypes:'{{route("workflow.temp-scan.doc-types")}}',store:'{{route("workflow.temp-scan.store")}}',data:'{{route("workflow.temp-scan.data")}}',exportLogs:'{{route("workflow.temp-scan.export.logs")}}',supportStore:(id)=>`/workflow/temp-scan/${id}/supporting`,supportList:(id)=>`/workflow/temp-scan/${id}/support-list`,finalSubmit:(id)=>`/workflow/temp-scan/${id}/final-submit`,destroy:(id)=>`/workflow/temp-scan/${id}`,supportDel:(id,sid)=>`/workflow/temp-scan/${id}/support/${sid}`,scanDetail:(id)=>`/workflow/temp-scan/${id}`};
let activeScan=null;
...
dt.on('draw',function(){const $p=$('#scansTable_wrapper .dataTables_paginate').first();const $i=$('#scansTable_wrapper .dataTables_info').first();if($p.length)$p.appendTo('#dtPaginate');if($i.length)$i.appendTo('#dtInfo')});
function buildBtns(r){let h=`<div class="dt-actions">`;h+=`<button class="dt-btn blue btn-view-modal" title="View Scan" data-id="${r.Scan_Id}"><svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg></button>`;h+=`<button class="dt-btn blue btn-s2" title="Supporting Files" data-id="${r.Scan_Id}" data-file="${esc(r.File)}" data-url="${esc(r.File_Location)}" data-date="${esc(r.Temp_Scan_Date)}"><svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.586a4 4 0 00-5.656-5.656l-6.415 6.585a6 6 0 108.486 8.486L20.5 13"/></svg></button>`;if(r.Final_Submit!=='Y')h+=`<button class="dt-btn green btn-fs" title="Final Submit" data-id="${r.Scan_Id}"><svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg></button>`;h+=`<button class="dt-btn red btn-del" title="Delete" data-id="${r.Scan_Id}"><svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg></button>`;return h+'</div>'}
