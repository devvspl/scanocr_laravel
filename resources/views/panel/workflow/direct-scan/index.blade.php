                    ...
                    <form id="uploadForm" class="flex flex-col gap-3.5" novalidate>
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            <div><label class="block text-xs font-medium text-stone-600 mb-1">Bill Date/Voucher
                                    Date <span class="text-red-500">*</span></label><input type="date" id="bill-date" onfocus="this.showPicker()"
...
@push('scripts')
    <script>
        $(function () {
            ...
            async function openViewModal(scanId) { try { const [supportRes] = await Promise.all([$.getJSON(R.supportList(scanId))]); const scanData = dt.rows().data().toArray().find(r => r.Scan_Id == scanId); if (!scanData) { alert('Scan not found'); return } $('#modalScanTitle').text(`Scan #${scanId}`); $('#modalScanMeta').text(scanData.Scan_Date || '—'); const mainUrl = scanData.File_Location; const mainName = scanData.File; window.__modalMainUrl = mainUrl; window.__modalMainName = mainName; if (supportRes.data.length) { $('#tabSupporting').show(); $('#tabSupportingCount').text(`(${supportRes.data.length})`); const grouped = {}; supportRes.data.forEach(f => { const g = f.doc_type_name || 'Other'; if (!grouped[g]) grouped[g] = []; grouped[g].push(f) }); let html = ''; Object.keys(grouped).forEach(gn => { html += `<div class="file-group-label">${esc(gn)} (${grouped[gn].length})</div>`; grouped[gn].forEach(f => { html += `<div class="file-link" data-url="${esc(f.File_Location)}" data-name="${esc(f.File)}"><span class="file-ext">${esc(f.File_Ext || '?')}</span><span>${esc(f.File)}</span></div>` }) }); $('#tabSupportingPanel').html(html) } else { $('#tabSupporting').hide(); $('#tabSupportingPanel').hide().empty() } $('#tabMainScan').addClass('active'); $('#tabSupporting').removeClass('active'); $('#tabSupportingPanel').removeClass('open'); $('#viewScanModal,#viewScanModalBackdrop').addClass('open'); loadModalViewer(mainUrl, mainName) } catch (e) { console.error(e); alert('Failed to load scan details') } }
            $(document).on('click', '#tabMainScan', function () { $('.modal-tab').removeClass('active'); $(this).addClass('active'); $('#tabSupportingPanel').removeClass('open'); loadModalViewer(window.__modalMainUrl, window.__modalMainName) });
            $(document).on('click', '#tabSupporting', function () { $('.modal-tab').removeClass('active'); $(this).addClass('active'); $('#tabSupportingPanel').toggleClass('open') });
            $(document).on('click', '#tabSupportingPanel .file-link', function () { const url = $(this).data('url'); const name = $(this).data('name'); $('#tabSupportingPanel .file-link').removeClass('active'); $(this).addClass('active'); loadModalViewer(url, name); $('#tabSupportingPanel').removeClass('open') });
            ...
            $('#btnCloseModal,#viewScanModalBackdrop').on('click', function () { $('#viewScanModal,#viewScanModalBackdrop').removeClass('open'); $('#tabSupportingPanel').removeClass('open').empty(); $('#tabSupporting').hide(); window.__modalMainUrl = null; window.__modalMainName = null });
