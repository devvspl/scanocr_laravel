...
@push('scripts')
    <script>
        $(function () {
            ...
            $(document).on('click', '.btn-del-sup', async function () { if (!activeScan || !(await customConfirm('Remove this supporting file?',{title:'Remove File',confirmText:'Remove',type:'danger'}))) return; const sid = $(this).data('sid'); await $.ajax({ url: R.supportDel(activeScan.id, sid), method: 'DELETE', headers: { 'X-CSRF-TOKEN': CSRF } }); $(`#sf-${sid}`).slideUp(150, function () { $(this).remove(); const n = $('#supportList .sf-row').length; const $badge = $('#supportCountBadge'); if (n) { $badge.text(n) } else { $badge.addClass('hidden'); $('#supportList').html(`<div class="flex flex-col items-center justify-center py-10 gap-2 text-stone-400"><svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.2" d="M9 13h6m-3-3v6m-9 1V7a2 2 0 012-2h6l2 2h6a2 2 0 012 2v8 a2 2 0 01-2 2H5a2 2 0 01-2-2z"/></svg><p class="text-xs">No supporting files yet</p></div>`) } }) });
            $('#btnFinalSubmit').on('click', async function () { if (!activeScan || !(await customConfirm('Mark this scan as final submitted?',{title:'Final Submit',confirmText:'Submit',type:'warning'}))) return; await $.ajax({ url: R.finalSubmit(activeScan.id), method: 'POST', headers: { 'X-CSRF-TOKEN': CSRF } }); dt.ajax.reload(null, false); $('#btnBackToStep1').click() });
