{{-- Template 2: Modern Blue — white base, thin blue accent lines, bordered invoice box --}}
<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
...
/* Top accent line */
.accent-line { border-top:3px solid #2563eb; margin:0; }

.inner { padding:28px 34px; }

/* Header */
.hdr { display:table; width:100%; margin-bottom:22px; }
...
.co-name { font-size:18px; font-weight:bold; color:#1e293b; }
.co-legal { font-size:10px; color:#94a3b8; margin-top:1px; }
.co-addr  { font-size:10px; color:#475569; line-height:1.6; margin-top:4px; }
.co-gstin { font-size:9.5px; color:#64748b; margin-top:4px; font-family:DejaVu Sans Mono,monospace; }

.inv-box { border:2px solid #2563eb; padding:10px 16px; display:inline-block; text-align:right; }
.inv-box-title { font-size:20px; font-weight:bold; color:#2563eb; letter-spacing:2px; text-transform:uppercase; }
.inv-box-num   { font-size:11px; color:#1e293b; margin-top:3px; font-family:DejaVu Sans Mono,monospace; }
.inv-box-date  { font-size:10px; color:#475569; margin-top:2px; }

/* Badge */
.badge { display:inline-block; padding:3px 10px; border-radius:2px; font-size:9px; font-weight:bold; text-transform:uppercase; letter-spacing:0.5px; margin-top:6px; border:1px solid #cbd5e1; color:#475569; }

/* Divider */
.rule { border:none; border-top:1px solid #e2e8f0; margin:12px 0; }

/* Meta */
.meta-grid { display:table; width:100%; margin-bottom:14px; }
.meta-cell { display:table-cell; vertical-align:top; padding-right:12px; }
.meta-lbl  { font-size:9px; font-weight:bold; color:#2563eb; text-transform:uppercase; letter-spacing:1px; margin-bottom:3px; }
.meta-val  { font-size:11px; color:#1e293b; line-height:1.5; }
.meta-bold { font-size:12px; font-weight:bold; color:#1e293b; }
.meta-mono { font-family:DejaVu Sans Mono,monospace; font-size:9.5px; color:#64748b; margin-top:3px; }

/* Table */
.tbl { width:100%; border-collapse:collapse; }
.tbl thead tr { border-bottom:2px solid #2563eb; }
.tbl th { padding:8px 5px; font-size:9px; font-weight:bold; color:#2563eb; text-transform:uppercase; letter-spacing:0.5px; background:#fff; }
.tbl th.r { text-align:right; } .tbl th.l { text-align:left; }
.tbl td { padding:8px 5px; font-size:11px; border-bottom:1px solid #f1f5f9; vertical-align:top; }
...
.desc-main { font-weight:bold; font-size:11px; color:#1e293b; }
.desc-sub  { font-size:9.5px; color:#94a3b8; margin-top:1px; }

/* Bottom */
.bot { display:table; width:100%; border-top:2px solid #2563eb; }
.bot-notes  { display:table-cell; width:55%; vertical-align:top; padding:14px 16px 14px 0; }
.bot-totals { display:table-cell; width:45%; vertical-align:bottom; padding:14px 0 14px 16px; border-left:1px solid #e2e8f0; }
.note-lbl { font-size:9px; font-weight:bold; color:#2563eb; text-transform:uppercase; letter-spacing:1px; margin-bottom:3px; }
.note-txt  { font-size:10px; color:#475569; line-height:1.5; margin-bottom:10px; }
.tot-row { display:table; width:100%; margin-bottom:4px; }
.tot-l { display:table-cell; font-size:11px; color:#475569; }
.tot-r { display:table-cell; font-size:11px; color:#1e293b; font-weight:500; text-align:right; }
.tot-r-red { color:#b91c1c; } .tot-r-grn { color:#15803d; }
.grand-sep { border:none; border-top:2px solid #2563eb; margin:6px 0; }
.grand-row { display:table; width:100%; }
.grand-l { display:table-cell; font-size:13px; font-weight:bold; color:#2563eb; }
.grand-r { display:table-cell; font-size:15px; font-weight:bold; color:#1e293b; text-align:right; }

/* Footer */
.footer { display:table; width:100%; margin-top:18px; padding-top:8px; border-top:1px solid #e2e8f0; }
.foot-l   { display:table-cell; font-size:9px; color:#94a3b8; vertical-align:bottom; }
.foot-sig { display:table-cell; width:160px; text-align:center; vertical-align:bottom; }
.sig-line { border-top:1px solid #2563eb; margin-top:36px; padding-top:4px; font-size:9px; color:#475569; }
</style>
</head>
<body>
<div class="page">
  ...
  <div class="inner">
    <div class="hdr">
      ...
      <div class="hdr-r">
        <div class="inv-box">
          ...
          <div class="inv-box-num">{{ $invoice->invoice_number }}</div>
          <div class="inv-box-date">{{ $invoice->invoice_date->format('d M Y') }}</div>
        </div>
    ...
    <table class="tbl">
      ...
      <tbody>
        ...
        <tr>
          ...
          <td class="mono" style="color:#475569;">{{ $item->hsn_sac ?: '—' }}</td>
          ...
          <td class="r mono" style="color:#94a3b8;">{{ $item->discount_amount>0 ? number_format((float)$item->discount_amount,2) : '—' }}</td>
          <td class="r mono" style="font-weight:bold;">{{ number_format((float)$item->taxable_amount,2) }}</td>
          <td class="r mono" style="color:#475569;">@if($invoice->is_igst){{ number_format((float)$item->igst_rate,2) }}%@else{{ number_format((float)$item->cgst_rate+(float)$item->sgst_rate,2) }}%@endif</td>
    ...
    <div class="bot">
      <div class="bot-notes">
        ...
        @if($invoice->narration)<div class="note-lbl">Narration</div><div class="note-txt" style="font-style:italic;color:#94a3b8;">{{ $invoice->narration }}</div>@endif
      </div>
    ...
    <div class="footer">
      ...
      <div class="foot-sig">
        ...
        @if($sigFullPath && file_exists($sigFullPath))
          ...
          <div style="font-size:9px;color:#475569;">{{ $signedLog->user->name ?? '' }}</div>
        @endif
