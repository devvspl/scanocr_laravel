...
@section('breadcrumb')
    ...
    <span class="text-stone-600">Proforma</span>
@endsection

@section('content')
<div x-data="proformaList()" x-init="init()">
    ...
    <div class="bg-white border border-stone-200 rounded-1xl overflow-hidden mb-4">
        <div class="px-4 py-2.5 flex items-center justify-between gap-3 min-h-[52px]">
            ...
            <a href="{{ route('sales.proforma.create') }}" class="tb-btn tb-btn-add shrink-0">
    ...
    <div class="bg-white border border-stone-200 rounded-1xl overflow-hidden">
        <div class="overflow-x-auto">
            <table id="proforma-table" class="w-full">
                <thead><tr>
                    <th style="width:140px;">Proforma #</th>
                    <th style="width:110px;">Date</th>
                    <th>Customer</th>
                    <th style="width:130px;" class="text-right">Amount</th>
                    <th style="width:100px;" class="dt-center">Status</th>
                    <th style="width:130px;" class="dt-center">Actions</th>
                </tr></thead>
                <tbody></tbody>
            </table>
        </div>
        ...
        <div id="empty-state" class="hidden flex-col items-center justify-center py-20 text-center">
            ...
            <p class="text-sm font-semibold text-stone-600">No proforma invoices yet</p>
            <p class="text-xs text-stone-400 mt-1 mb-4">Create your first proforma invoice to get started.</p>
            <a href="{{ route('sales.proforma.create') }}" class="tb-btn tb-btn-add">
                <svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M12 4v16m8-8H4"/></svg>
                New Proforma
            </a>
        </div>
    ...
    <div x-show="rejectModal.open" x-cloak
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-6"
            ...
            <h3 class="text-sm font-semibold text-stone-800 mb-4">Reject Proforma</h3>
    ...
    <div x-show="cancelModal.open" x-cloak
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-6"
             x-transition:enter="transition ease-out duration-150"
             x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100">
            <h3 class="text-sm font-semibold text-stone-800 mb-4">Cancel Proforma</h3>
        ...
        <div class="relative w-full max-w-md bg-white shadow-2xl flex flex-col overflow-hidden"
            ...
            <div class="px-5 py-4 border-b border-stone-100 flex items-center justify-between shrink-0">
                <div>
                    ...
                    <p class="text-[11px] text-stone-400 mt-0.5" x-text="'Proforma #' + approvalPanel.invoiceNumber"></p>
                </div>
                ...
                <div x-show="!approvalPanel.loading && approvalPanel.setting">
                    ...
                    <div x-show="approvalPanel.invoice?.status === 'submitted'" class="border-t border-stone-100 pt-4">
                        ...
                        <div class="flex gap-2">
                            <button @click="panelApprove()" class="flex-1 inline-flex items-center justify-center gap-1.5 h-9 px-4 text-xs font-semibold text-white rounded-lg transition-colors" style="background:#16a34a;">Approve</button>
                            <button @click="panelReject()" class="flex-1 inline-flex items-center justify-center gap-1.5 h-9 px-4 text-xs font-semibold text-white rounded-lg transition-colors" style="background:#dc2626;">Reject</button>
                        </div>
                    </div>
                </div>
            </div>
...
function proformaList() {
    return {
        ...
        async openApprovalPanel(id, number) {
            this.approvalPanel = { open: true, loading: true, invoiceId: id, invoiceNumber: number || '', invoice: null, setting: null, logs: [], remarks: '' };
            try {
                const res = await fetch(`/sales/proforma/${id}/approval-logs`, { headers: { 'Accept': 'application/json' } });
        ...
        async panelApprove() {
            ...
            const res = await fetch(`/sales/proforma/${id}/level-approve`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content, 'Accept': 'application/json' },
                body: JSON.stringify({ remarks: this.approvalPanel.remarks }),
            });
            const json = await res.json();
            if (json.success) { _showGlobalToast('success', json.message); window._proformaTable?.ajax.reload(null, false); this.openApprovalPanel(id, this.approvalPanel.invoiceNumber); }
        ...
        async panelReject() {
            ...
            const id = this.approvalPanel.invoiceId;
            const res = await fetch(`/sales/proforma/${id}/level-reject`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content, 'Accept': 'application/json' },
                body: JSON.stringify({ remarks: this.approvalPanel.remarks }),
            });
        ...
        async confirmReject() {
            ...
            await this.action(`/sales/proforma/${this.rejectModal.id}/reject`, 'POST', { reason: this.rejectModal.reason });
            this.rejectModal.open = false;
        },

        async confirmCancel() {
            await this.action(`/sales/proforma/${this.cancelModal.id}/cancel`, 'POST', { reason: this.cancelModal.reason });
        ...
        async action(url, method, body = {}) {
            ...
            const json = await res.json();
            if (json.success) {
                window._proformaTable?.ajax.reload(null, false);
...
$(function () {
    ...
    const table = $('#proforma-table').DataTable({
        serverSide: true,
        processing: true,
        ajax: {
            url: '{{ route("sales.proforma.data") }}',
            data(d) {
        ...
        columns: [
            {
                ...
                render(v, t, row) {
                    return `<a href="/sales/proforma/${row.id}" class="font-mono text-xs font-semibold text-red-700 hover:underline">${v}</a>`;
                },
            },
            { data: 'proforma_date', className: 'td-date text-xs' },
            {
                data: 'party_name',
                render(v, t, row) {
            ...
            },
            {
                ...
                className: 'dt-center',
                render(v, t, row) {
                    let badge = `<span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold ${row.status_class}">${row.status_label}</span>`;
                    if (row.is_converted) badge += `<span class="inline-flex items-center px-1.5 py-0.5 rounded-full text-[9px] font-semibold bg-purple-50 text-purple-700 ml-1">Converted</span>`;
                    return badge;
                },
            },
            {
                ...
                render(id, t, row) {
                    ...
                    btns += `<a href="/sales/proforma/${id}" class="act-btn act-edit" title="View">
                    ...
                    if (row.status === 'submitted' || row.status === 'approved' || row.status === 'rejected') {
                        btns += `<button class="act-btn btn-approval" data-id="${id}" data-number="${row.proforma_number}" title="Approval" style="color:#7c3aed">
                            <svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"/></svg>
                        </button>`;
                    }

                    if (row.can_edit) {
                        btns += `<a href="/sales/proforma/${id}/edit" class="act-btn act-edit" title="Edit">
                    ...
                    if (row.can_convert) {
                        btns += `<button class="act-btn btn-convert" data-id="${id}" title="Convert to Invoice" style="color:#7c3aed">
        ...
        drawCallback() {
            ...
            if (total === 0) {
                $('#proforma-table').closest('.overflow-x-auto').hide();
                $('#empty-state').removeClass('hidden').addClass('flex');
            } else {
                $('#proforma-table').closest('.overflow-x-auto').show();
                $('#empty-state').removeClass('flex').addClass('hidden');
            }
        },
    });

    window._proformaTable = table;

    $('#filter-status, #filter-date-from, #filter-date-to').on('change', () => table.ajax.reload(null, false));

    $('#proforma-table').on('click', '.btn-submit', async function () {
        if (!confirm('Submit this proforma for approval?')) return;
        const id  = $(this).data('id');
        const res = await fetch(`/sales/proforma/${id}/submit`, { method: 'POST', headers: { 'X-CSRF-TOKEN': CSRF, 'Accept': 'application/json' } });
        const json = await res.json();
        if (json.success) { table.ajax.reload(null, false); _showGlobalToast('success', json.message); }
        else _showGlobalToast('error', json.message);
    });

    $('#proforma-table').on('click', '.btn-convert', async function () {
        if (!confirm('Convert this proforma to a sales invoice?')) return;
        const id  = $(this).data('id');
        const res = await fetch(`/sales/proforma/${id}/convert`, { method: 'POST', headers: { 'X-CSRF-TOKEN': CSRF, 'Accept': 'application/json' } });
        const json = await res.json();
        if (json.success) {
            _showGlobalToast('success', json.message);
            if (json.redirect) setTimeout(() => window.location.href = json.redirect, 1000);
            else table.ajax.reload(null, false);
        } else _showGlobalToast('error', json.message);
    });

    $('#proforma-table').on('click', '.btn-reject', function () {
        const id = $(this).data('id');
        document.querySelector('[x-data="proformaList()"]')?._x_dataStack?.[0]?.openReject(id);
    });

    $('#proforma-table').on('click', '.btn-delete', async function () {
        if (!confirm('Delete this draft proforma? This cannot be undone.')) return;
        const id  = $(this).data('id');
        const res = await fetch(`/sales/proforma/${id}`, { method: 'DELETE', headers: { 'X-CSRF-TOKEN': CSRF, 'Accept': 'application/json' } });
        const json = await res.json();
        if (json.success) { table.ajax.reload(null, false); _showGlobalToast('success', json.message); }
        else _showGlobalToast('error', json.message);
    });

    $('#proforma-table').on('click', '.btn-approval', function () {
        const id = $(this).data('id');
        const num = $(this).data('number');
        const comp = document.querySelector('[x-data="proformaList()"]')?._x_dataStack?.[0];
