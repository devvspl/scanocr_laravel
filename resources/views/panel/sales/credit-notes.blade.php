...
@section('breadcrumb')
    ...
    <span class="text-stone-600">Credit Notes</span>
@endsection

@section('content')
<div x-data="creditNoteList()" x-init="init()">
    ...
    <div class="bg-white border border-stone-200 rounded-1xl overflow-hidden mb-4">
        <div class="px-4 py-2.5 flex items-center justify-between gap-3 min-h-[52px]">
            ...
            <a href="{{ route('sales.credit-notes.create') }}" class="tb-btn tb-btn-add shrink-0">
    ...
    <div class="bg-white border border-stone-200 rounded-1xl overflow-hidden">
        <div class="overflow-x-auto">
            <table id="credit-notes-table" class="w-full">
                <thead><tr>
                    <th style="width:140px;">Credit Note #</th>
        ...
        <div id="empty-state" class="hidden flex-col items-center justify-center py-20 text-center">
            ...
            <p class="text-sm font-semibold text-stone-600">No credit notes yet</p>
            <p class="text-xs text-stone-400 mt-1 mb-4">Create your first credit note to issue refunds or adjustments.</p>
            <a href="{{ route('sales.credit-notes.create') }}" class="tb-btn tb-btn-add">
                <svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M12 4v16m8-8H4"/></svg>
                New Credit Note
            </a>
        </div>
...
function creditNoteList() {
    return {
        ...
        async confirmReject() {
            await this.action(`/sales/credit-notes/${this.rejectModal.id}/reject`, 'POST', { reason: this.rejectModal.reason });
            this.rejectModal.open = false;
        },

        async confirmCancel() {
            await this.action(`/sales/credit-notes/${this.cancelModal.id}/cancel`, 'POST', { reason: this.cancelModal.reason });
        ...
        async action(url, method, body = {}) {
            ...
            if (json.success) {
                window._creditNotesTable?.ajax.reload(null, false);
...
$(function () {
    ...
    const table = $('#credit-notes-table').DataTable({
        ...
        processing: true,
        ajax: {
            url: '{{ route("sales.credit-notes.data") }}',
            data(d) {
        ...
        columns: [
            {
                ...
                render(v, t, row) {
                    return `<a href="/sales/credit-notes/${row.id}" class="font-mono text-xs font-semibold text-red-700 hover:underline">${v}</a>`;
                },
            },
            { data: 'credit_note_date', className: 'td-date text-xs' },
            {
                data: 'party_name',
            ...
            },
            {
                data: 'grand_total',
                className: 'dt-right',
                render(v) { return `<span class="font-mono text-xs font-semibold text-stone-800">${parseFloat(v).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>`; },
            },
            {
                data: 'reason_label',
                render(v) {
                    return v ? `<span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold bg-stone-100 text-stone-600">${v}</span>` : '';
                },
            },
            {
                data: 'status',
            ...
            },
            {
                ...
                render(id, t, row) {
                    ...
                    btns += `<a href="/sales/credit-notes/${id}" class="act-btn act-edit" title="View">
                    ...
                    if (row.can_edit) {
                        btns += `<a href="/sales/credit-notes/${id}/edit" class="act-btn act-edit" title="Edit">
        ...
        drawCallback() {
            ...
            if (total === 0) {
                $('#credit-notes-table').closest('.overflow-x-auto').hide();
                $('#empty-state').removeClass('hidden').addClass('flex');
            } else {
                $('#credit-notes-table').closest('.overflow-x-auto').show();
                $('#empty-state').removeClass('flex').addClass('hidden');
            }
        },
    });

    window._creditNotesTable = table;

    $('#filter-status, #filter-date-from, #filter-date-to').on('change', () => table.ajax.reload(null, false));

    $('#credit-notes-table').on('click', '.btn-submit', async function () {
        if (!confirm('Submit this credit note for approval?')) return;
        const id  = $(this).data('id');
        const res = await fetch(`/sales/credit-notes/${id}/submit`, { method: 'POST', headers: { 'X-CSRF-TOKEN': CSRF, 'Accept': 'application/json' } });
        const json = await res.json();
    ...
    $('#credit-notes-table').on('click', '.btn-delete', async function () {
        if (!confirm('Delete this draft credit note? This cannot be undone.')) return;
        const id  = $(this).data('id');
        const res = await fetch(`/sales/credit-notes/${id}`, { method: 'DELETE', headers: { 'X-CSRF-TOKEN': CSRF, 'Accept': 'application/json' } });
        const json = await res.json();
        if (json.success) { table.ajax.reload(null, false); _showGlobalToast('success', json.message); }
        else _showGlobalToast('error', json.message);
    });
