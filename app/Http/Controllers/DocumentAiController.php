...
class DocumentAiController extends Controller
{
    ...
    /**
     * PAGE: Odometer Playground
     */
    public function odometerPlayground()
    {
        return view('document-ai.odometer');
    }

    /**
     * AJAX: Extract Odometer Reading
     */
    public function extractOdometer(Request $request)
    {
        set_time_limit(300);

        $request->validate([
            'file'           => 'required|file|mimes:jpg,jpeg,png|max:10240',
            'odometer_type'  => 'nullable|in:auto,digital,analog',
        ]);

        $file       = $request->file('file');
        $ext        = strtolower($file->getClientOriginalExtension());
        $stored     = time() . '_' . uniqid() . '.' . $ext;
        $uploadPath = storage_path('app/public/odometer');

        if (!is_dir($uploadPath)) mkdir($uploadPath, 0755, true);
        if (!is_dir($uploadPath . '/crops')) mkdir($uploadPath . '/crops', 0755, true);

        $file->move($uploadPath, $stored);
        $storedPath = $uploadPath . DIRECTORY_SEPARATOR . $stored;

        $record = \App\Models\OdometerReading::create([
        ...
        // Try Python first, fallback to PHP
        $type = $request->input('odometer_type', 'auto');
        $result = $this->tryOdometerPython($storedPath, $type);

        if (!$result) {
            $record->update(['status' => 'failed']);
            return response()->json([
                'success' => false,
                'message' => 'Odometer extraction not available. Python dependencies required.',
            ], 500);
        }
        ...
        if (!empty($result['cropped_image_path'])) {
            $croppedFilename = basename($result['cropped_image_path']);
        }

        $record->update([
            'odometer_type'         => $result['odometer_type'] ?? 'unknown',
            'raw_ocr_text'          => $result['raw_ocr_text'] ?? null,
            'reading_value'         => $result['reading'] ?? null,
            'reading_unit'          => $result['unit'] ?? 'km',
            'ocr_confidence'        => $result['ocr_confidence'] ?? null,
            'extraction_confidence' => $result['confidence'] ?? null,
            'is_valid_range'        => $result['validation']['is_valid'] ?? null,
            'validation_message'    => $result['validation']['message'] ?? null,
            'bounding_box'          => $result['bounding_box'] ?? null,
            'cropped_filename'      => $croppedFilename,
            'status'                => 'extracted',
        ]);
        ...
        return response()->json([
            'success'       => true,
            'record_id'     => $record->id,
            'file_url'      => asset('storage/odometer/' . $stored),
            'crop_url'      => $croppedFilename ? asset('storage/odometer/crops/' . $croppedFilename) : null,
            'odometer_type' => $result['odometer_type'],
    ...
    /**
     * AJAX: Confirm Odometer Reading
     */
    public function confirmOdometerReading(Request $request)
    {
        $request->validate([
            'record_id'         => 'required|integer|exists:odometer_readings,id',
            'confirmed_reading' => 'required|numeric|min:0|max:999999',
            'confirmed_unit'    => 'required|in:km,miles',
            'add_to_training'   => 'nullable|boolean',
            'user_remark'       => 'nullable|string|max:500',
        ]);

        $record = \App\Models\OdometerReading::findOrFail($request->record_id);
        $status = ($record->reading_value == $request->confirmed_reading) ? 'confirmed' : 'corrected';

        $record->update([
            'confirmed_reading' => $request->confirmed_reading,
            'confirmed_unit'    => $request->confirmed_unit,
            'status'            => $status,
            'user_remark'       => $request->user_remark,
        ]);

        if ($request->boolean('add_to_training')) {
            \App\Models\OdometerTrainingData::create([
                'odometer_type' => $record->odometer_type,
                'source_type'   => $record->source_type ?? 'dashboard_photo',
                'true_reading'  => $request->confirmed_reading,
                'true_unit'     => $request->confirmed_unit,
                'ocr_raw_text'  => $record->raw_ocr_text,
                'status'        => 'active',
                'created_by'    => auth()->id(),
            ]);
            $record->update(['added_to_training' => true]);
        }

        return response()->json(['success' => true, 'message' => 'Reading ' . $status . ' successfully.', 'status' => $status]);
    }

    /**
     * Try Python odometer extraction (new pipeline with YOLO + VGG16 + OCR)
     */
    private function tryOdometerPython(string $storedPath, string $type): ?array
    {
        if (env('DOCUMENT_AI_PHP_ONLY', false)) {
            return null;
        }

        $pipelinePath = $this->pythonPath . DIRECTORY_SEPARATOR . 'odometer' . DIRECTORY_SEPARATOR . 'pipeline.py';
        ...
        $cmd = sprintf(
            '%s %s --file %s --save-crop 1 --type %s',
            $this->pythonBin,
            escapeshellarg($pipelinePath),
            escapeshellarg($storedPath),
            escapeshellarg($type)
        );

        $descriptors = [0 => ['pipe', 'r'], 1 => ['pipe', 'w'], 2 => ['pipe', 'w']];
        $cwd = dirname($pipelinePath);
        ...
        if (empty($rawOutput)) return null;
